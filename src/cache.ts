...
async function getFromCacheCompressImage(
  ...
  try {
    ...
    const format = JSON.parse((await fs.readFile(path.join(location, 'format'))).toString()) as ImageFormat;  
...
async function addToCacheCompressImage(hash: string, image: Image): Promise<void> {
    if ($state.args.nocache) {
        return;
    }
